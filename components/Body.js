...
const Body = ({ chooseTrack, spotifyApi }) => {
   ...
   return (
      <section className="ml-20 pl-6 bg-black py-3 space-y-8 px-1 md:max-w-6xl flex-grow md:mr-2.5 z-50">
         <Search search={search} setSearch={setSearch} />
         <div className="grid overflow-y-scroll scrollbar-hide h-96 py-2 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-8 p-4">
            {searchResults.length === 0
               ? newReleases.slice(11,15).map((track) => (
                    <Poster
                       key={track.id}
                       track={track}
